...
build.plot <- function(plot.data){
  ...
  p <- ggplot(dt.polygons) + 
    ...
    theme(panel.spacing.y = unit(0, "lines"),
          ...
          plot.title = element_text(family = "Apercu Pro" ),
...
# Clean Data --------------------------------------------------------------
x <- fread(here::here('Euro_2020',  "E2.csv"))

width = .35 # width of bars


teams <- unique(c(x$HomeTeam, x$AwayTeam))
...
source(here::here('Euro_2020','team_colours.R'))
...
wrap_plots(all.p,ncol = 1, heights = 0.5) + 
  ggsave(filename = here::here('Euro_2020','2020.png'),
